...
def ErrorsCalculator(symbols, values, errors, function):
	...
	for n in range(len(S)):
		sElement = 'S['+str(n)+']'
		function = function.replace(symbols[n], sElement)
	...
	for i in range(len(symbols)):
		Error_sq += ((function.diff(symbols[i])*errors[i])**2)

	Error_re = 'Error_sq.evalf(subs={symbols[0]:values[0]'

	for x in range(1,len(symbols)):
		Error_re += ', symbols['+ str(x) + ']:values[' + str(x) + ']'
	Error_re += '})'
...
def derivative(table, index, columns):
	...
	try:
		...
		index[max(index.keys())] = "Derivative 1"
	except KeyError:
		table["Derivative"] = derivative
		index[max(index.keys())] = "Derivative"
