...
class GUI(QMainWindow):
	...
	def menuBarsus(self):
		...
		savepng.triggered.connect(self.savefig)
	...
	def savefig(self):
		fname = str(QFileDialog.getSaveFileName(self, 'Open file', '/home/jaime/', "Image files (*.png)"))
		fname = fname.split(',')[0]
		fname = fname.split('(u')
		fname = fname[1].split("'")
		file = fname[1]
		PlotCanvas().savePlot(file)
...
class PlotCanvas(FigureCanvas):
	...
	def savePlot(self, text):
		dpi = 172
		self.fig.savefig(text, bbox_inches='tight')
